...
/**
 * 直接复制了CopyOnwrite的源码改造使用
 * 否则因为可见性问题，继承改造很麻烦
 * 需要反射修改setAccessible等
 * 
 * 注释了Unsafe部分 否则无法调用
 * 
 * @author zhangtianyi
 ...
 * @param <E>
 */
public class MyCopyOnWriteArrayList<E> implements List<E>, RandomAccess, Cloneable, java.io.Serializable {
...
//			clone.resetLock();
	 ...
	 *             "../Collection.html#optional-restrictions">optional</a>), or
	 *             if the specified collection is null
	 * @see #remove(Object)
	 */
	...
	public boolean addAll(Collection<? extends E> c) {
		Object[] cs = (c.getClass() == MyCopyOnWriteArrayList.class) ? ((MyCopyOnWriteArrayList<?>) c).getArray()
				: c.toArray();
		if (cs.length == 0)
			return false;
		final ReentrantLock lock = this.lock;
