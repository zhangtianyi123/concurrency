...
/**
 * ThreadPoolExecutor的execute方法是直接执行没有返回值的
 * 通过submit方法和FutureTask工具类获取执行结果
 * 
 * Future:取消任务，判断任务是否已取消，判断任务是否已结束，（超时的）获取任务结束
 * 使用模型往往类似于join  也类似于countdownlatch  参见CountDownLatchBase
 * 
...
public class FutureBase {
	...
	void testOneDependencyMoreByFuture() throws InterruptedException, ExecutionException {
		...
		FutureTask<Integer> futureTaskTwo = new FutureTask<>(() -> {return 2;});
		FutureTask<Integer> futureTaskThree = new FutureTask<>(() -> {return 3;});
	...
	/**
	 * future同比对比使用阻塞队列实现
	 * 当然也可以参照CountDownLatchBase.java中对此模型的两个实现
	 * @throws InterruptedException 
	 */
	void testOneDependencyMoreByBlockingQueue() throws InterruptedException {
		...
		es.execute(() -> { cache.add(2); });
		es.execute(() -> { cache.add(3); });
		
		int oneResult = (int) cache.take();
		int anothorResult = (int) cache.take();
		
		
		System.out.println(oneResult * anothorResult);
	}
